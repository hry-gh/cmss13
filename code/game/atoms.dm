...
/atom/vv_do_topic(list/href_list)
		...
		if(reagents)
			...
			switch(tgui_alert(usr, "Choose a method.", "Add Reagents", list("Search", "Choose from a list", "I'm feeling lucky")))
				...
				if("Choose from a list")
					chosen_id = input(usr, "Choose a reagent to add.", "Choose a reagent.") in sort_list(subtypesof(/datum/reagent), /proc/cmp_typepaths_asc)
	...
	if(href_list[VV_HK_TRIGGER_EXPLOSION] && check_rights(R_ADMIN))
		if(!isobj(src) && !ismob(src) && !isturf(src))
			to_chat(usr, "This can only be done to instances of type /obj, /mob and /turf")
			return

		cell_explosion(src, 150, 100, , create_cause_data("divine intervention"))
		message_staff("[key_name(usr, TRUE)] has exploded [src]!")
