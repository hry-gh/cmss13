/obj/structure/machinery/computer/camera_advanced/shuttle_docker
	...
	var/list/locked_traits = list(ZTRAIT_RESERVED, ZTRAIT_ADMIN)
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/Initialize(mapload)
	. = ..()
	if(!mapload)
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/Destroy()
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/attack_hand(mob/user)
	if(jammed)
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/interact(mob/user)
	if(!allowed(user))
		return
	..()

///Change the fly state of the shuttle, called when a new shuttle port is reached
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/proc/shuttle_arrived()
	return

/obj/structure/machinery/computer/camera_advanced/shuttle_docker/give_actions(mob/living/user)
	if(jumpto_ports.len)
		jump_action = new /datum/action/innate/camera_jump/shuttle_docker
	..()
	if(rotate_action)
		rotate_action.target = user
		rotate_action.give_to(user)
		actions += rotate_action

	if(place_action)
		place_action.target = user
		place_action.give_to(user)
		actions += place_action

/obj/structure/machinery/computer/camera_advanced/shuttle_docker/CreateEye()
	shuttle_port = SSshuttle.getShuttle(shuttleId)
	if(QDELETED(shuttle_port))
		shuttle_port = null
		return
	shuttle_port.shuttle_computer = src
	eyeobj = new /mob/camera/eye/remote/shuttle_docker(null, src)
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/give_eye_control(mob/user)
	. = ..()
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/remove_eye_control(mob/living/user)
	. = ..()
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/proc/canDesignateTarget()
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/proc/rotateLandingSpot()
	var/mob/camera/eye/remote/shuttle_docker/the_eye = eyeobj
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/proc/check_hovering_spot(turf/next_turf)
	if(!next_turf)
		return
	var/mob/camera/eye/remote/shuttle_docker/the_eye = eyeobj
	var/list/image_cache = the_eye.placement_images
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/proc/checkLandingSpot()
	var/mob/camera/eye/remote/shuttle_docker/the_eye = eyeobj
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/proc/checkLandingTurf(turf/T, list/overlappers)
	...
	for(var/obj/O in T)
		if(CHECK_BITFIELD(O.flags_atom, DROPSHIP_IMMUNE))
			return SHUTTLE_DOCKER_BLOCKED
...
/obj/structure/machinery/computer/camera_advanced/shuttle_docker/connect_to_shuttle(obj/docking_port/mobile/port, obj/docking_port/stationary/dock)
	if(port)
		shuttleId = port.id
...
/mob/camera/eye/remote/shuttle_docker
	visible_icon = FALSE
	use_static = FALSE
...
/mob/camera/eye/remote/shuttle_docker/relaymove(mob/user, direct)
	var/turf/T = get_turf(get_step(src, direct))
	var/obj/structure/machinery/computer/camera_advanced/shuttle_docker/console = origin
...
/mob/camera/eye/remote/shuttle_docker/setLoc(T)
	..()
	var/obj/structure/machinery/computer/camera_advanced/shuttle_docker/console = origin
	console.checkLandingSpot()

/mob/camera/eye/remote/shuttle_docker/update_remote_sight(mob/living/user)
	var/obj/structure/machinery/computer/camera_advanced/shuttle_docker/console = origin
...
/datum/action/innate/shuttledocker_rotate/action_activate()
	if(QDELETED(target) || !isliving(target))
		return
	var/mob/living/C = target
	var/mob/camera/eye/remote/remote_eye = C.remote_control
	var/obj/structure/machinery/computer/camera_advanced/shuttle_docker/origin = remote_eye.origin
