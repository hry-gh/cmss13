	...
	switch(action)
		...
		if("mass_apply")
			if(!check_rights_for(usr.client, R_ADMIN))
				to_chat(usr, SPAN_DANGER("Requires Admin permissions."))
				return
			...
			for(var/thing in world.contents)
				if(istype(thing, target_path))
					...
					count += 1
			message_admins("[usr.ckey] JUST MASS FILTER EDITED [count] WITH PATH OF [params["path"]]!")
			log_admin("[usr.ckey] JUST MASS FILTER EDITED [count] WITH PATH OF [params["path"]]!")
